...
def main(argv):
    ...
    is_aggregateds = [False]
    sepset_rules = ['first']
    orientation_rules = ['majority']
    detect_rbos = [False]
    detect_post_rbos = [False]
    ...
    def writing_phase(_phase):
        ...
        with open(f'{working_dir}naive_CUT_phase_{_phase}_{from_index}_{to_index}_{identifier}.csv', 'a') as _f:
            while queue:
                vals = queue.popleft()
        ...
        for base_size in [200, 300, 400, 500]:  # 200, 300, 400,500, 600
            ...
            for is_aggregated, sepset_rule, orientation_rule, detect_rbo, detect_post_rbo in options:
                ...
                if p2_key not in done[2]:
                    ...
                    learner.CUT_based_collider_tests()
                    learner.naive_orient()
